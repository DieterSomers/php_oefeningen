    ...
    foreach ( $data as $row )
    {
        ...
        if ( key_exists( $fieldname, $_POST) )
        {
            ...
            if ( in_array( $type, explode("," , "VARCHAR,TEXT" ) ) )
            {
                ...
                //is de tekst een wachtwoord? En is dit niet te lang
                if ( $fieldname == "usr_password" )
                {
                    if( ! isPassword($sent_value) )
                    {
                        $msg = "Your password should have a minimum of 8 characters";
                        $_SESSION['errors'][ "$fieldname" . "_error" ] = $msg;
                    }
                }

                //id de teskst een email adres en is dit geldig?
                if ( $fieldname == "usr_email" )
                {
                    if( ! isEmail($sent_value) )
                    {
                        $msg = "This is not a valid email address";
                        $_SESSION['errors'][ "$fieldname" . "_error" ] = $msg;
                    }

                    //is dit email adres in gebruik
                    $mail_list = GetData("select usr_email from $table");
                    foreach ($mail_list as $rows)
                    {
                        if($rows["usr_email"] == $sent_value)
                        {
                            $msg = "This email address is allready in use";
                            $_SESSION['errors'][ "$fieldname" . "_error" ] = $msg;
                        }
                    }
